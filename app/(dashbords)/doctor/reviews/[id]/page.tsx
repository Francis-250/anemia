    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div className="space-y-4 lg:sticky lg:top-6">
          ...
          <div className="rounded-lg border p-4">
            <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-3">Risk factors & Clinical indicators</p>
            <div className="grid grid-cols-2 gap-2">
              <RiskFlag label="Low Iron Intake" active={patient?.smokingStatus ?? false} />
              <RiskFlag label="Heavy Menstruation" active={patient?.diabetic ?? false} />
              <RiskFlag label="Blood Loss History" active={patient?.hypertension ?? false} />
              <RiskFlag label="Chronic Disease" active={patient?.heartDisease ?? false} />
            </div>
          </div>
          ...
          <div className="rounded-lg border p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-semibold capitalize">{assessment.riskLevel.toLowerCase()} risk</p>
              </div>
          ...
          <div className="rounded-lg border p-4">
            <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-3">Risk indicator score</p>
            <div className="flex gap-1.5">
              {["N", "S", "B", "M"].map((letter, index) => (
            ...
            <p className="mt-2 text-xs text-muted-foreground">{assessment.fastScore} of 4 risk categories present</p>
          </div>
        </div>
